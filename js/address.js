...
import { setConfirmTooltip, showToast } from "./mixin.js";
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Set payment section
  if (Boolean(new URL(location.href).searchParams.get("payment"))) {
    document.querySelector(".progress-container").style.display = "flex";
    document.querySelector(".select-address-container").style.display = "flex";
...
document.querySelector("#main-address-form").addEventListener("submit", (e) => {
  ...
  if (
    ...
    putMainAddress(addressInfo);
    resetDefaultAddress(0);
...
document.querySelector("#add-form").addEventListener("submit", (e) => {
  ...
  postSubAddress(addressInfo);
  subAddresses = [...subAddresses, addressInfo];
  document.querySelector(".sub-address-container").innerHTML +=
    "<hr>" + setSubAddress(addressInfo);
  ...
  document.querySelector(".select-address-container").innerHTML += `
...
function setEditableEvent(container) {
  ...
  container.querySelector("form").addEventListener("submit", (e) => {
    ...
    document.querySelector(`#edit-container-${id}`).classList.remove("editing");
...
function resetDefaultAddress(id) {
  if (id !== 0 && mainAddress.isDefault) {
    putMainAddress({ ...mainAddress, isDefault: false });
    document.querySelector(
